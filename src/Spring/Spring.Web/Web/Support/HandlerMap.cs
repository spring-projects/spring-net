	...
	public class HandlerMap : IDictionary
	{
		...
		public HandlerMapEntry MapPath( string virtualPath )
		{
			...
			for(int i=0;i<this._internalTable.Count;i++)
			{
				...
				{
					if (Log.IsEnabled(LogLevel.Debug)) Log.Debug(string.Format("found mapping '{0}' for url '{1}'", handlerMapEntry, virtualPath));
					return handlerMapEntry;
				}
			}

			if (Log.IsEnabled(LogLevel.Debug)) Log.Debug(string.Format("no mapping found for url '{0}'", virtualPath));
			return null;
		}
