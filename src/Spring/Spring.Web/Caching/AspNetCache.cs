...
public class AspNetCache : AbstractCache
{
    ...
    public override void Remove(object key)
    {
        if (key != null)
        {
            if (Log.IsEnabled(LogLevel.Debug)) Log.LogDebug("removing item '{Key}' from cache '{CacheName}'", key, this._cacheName);
    ...
    public void Insert(object key, object value, TimeSpan timeToLive, bool slidingExpiration, CacheItemPriority itemPriority)
    {
        ...
        if (Log.IsEnabled(LogLevel.Debug)) Log.LogDebug("adding item '{Key}' to cache '{CacheName}'", key, this._cacheName);
